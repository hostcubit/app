...
class ContactSettingsController extends Controller
{
    ...
    public function index() {
        ...
        $meta_data = new CollectionService(collect(json_decode(auth()->user()->contact_meta_data, true)));
        ...
        return view('user.contact.settings.index', compact('title', 'meta_data'));
    }
    ...
    public function metaSave(ContactSettingsRequest $request) {
        ...
        $status  = 'success';
        $message = translate("New contact attribute added");
        try {

            $data = $request->all();
            unset($data['_token']);
            $final_data = $this->contactSettingService->save($data, auth()->user()->id);
            auth()->user()->contact_meta_data = json_encode($final_data["contact_meta_data"]);
            auth()->user()->update();
    ...
    */ 
    public function metaDelete(Request $request) {
        ...
        try {
            ...
            unset($data['_token']);
            $user = auth()->user();
            $final_data = $this->contactSettingService->delete($data, $user);
            $user->contact_meta_data = json_encode($final_data["contact_meta_data"]);
            $user->update();
