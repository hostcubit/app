...
class WhatsappDeviceController extends Controller {
    ...
    public function index() {
    	...
    	return view('user.gateway.whatsapp.device', compact('title', 'gateways', 'server_status'));
    }
   ...
   /**
    ...
    public function save(WhatsappDeviceRequest $request) {
        ...
        try {
            ...
            $whatsapp_device_count = WhatsappDevice::where("user_id", auth()->user()->id)->where("type", $type)->get()->count();
            ...
            if($type == StatusEnum::TRUE->status()) {
                ...
                if($access->is_allowed && ($whatsapp_device_count < $access->gateway_limit || $access->gateway_limit == 0)) { 
